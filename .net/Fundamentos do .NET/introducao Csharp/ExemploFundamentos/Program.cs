...
listaString.Add("SC");
//Ao esgotar a capacidade (que era 4), sua capacidade se espande em +4, e assim toda vez que esgotar.
...
{
    Console.WriteLine($"Posição n°: {contadorForeach} - valor {item}");
    contadorForeach++;
}
...
//Array.Resize(ref arrayInteiros, arrayInteiros.Length * 2); Aumenta a capacidade de um array já existente
int[] arrayInteirosDobrado = new int [arrayInteiros.Length * 2]; //Cria um array novo com base em um array já existente [arrayInteiros.Length * 2]
Array.Copy(arrayInteiros, arrayInteirosDobrado, arrayInteiros.Length); //Copia um array já existente.
